...
from flwr.server.app import ServerConfig, init_defaults, init_defaults_returns_parameters_server, run_fl, run_fl_return_parameters
...
def start_simulation_no_ray(
    *,
    ...
    """Start a Non-Ray-based Flower simulation server.
    ...
    parameters_list : List[Tuple[int,NDArrays]]
        List of parameters from each round of training.
    ...
    initialized_server, initialized_config = init_defaults_returns_parameters_server(
        server=server,
        config=config,
        strategy=strategy,
    ...
    # Register one FnClientProxy object for each client with the ClientManager
    for cid in cids:
        client_proxy = FnClientProxy(
            client_fn=client_fn,
            cid=cid,
        )
