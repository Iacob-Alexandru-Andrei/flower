...
class ReturnParametersServer(Server):
    """Flower server."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(client_manager=client_manager,
                         strategy=strategy)
        self.return_all_parameters = return_all_parameters
        self.return_params: List[Tuple[int, NDArrays]] = []
    ...
    def fit(self, num_rounds: int, timeout: Optional[float]) -> Tuple[List[Tuple[int, NDArrays]], History]:
        ...
        if not self.return_all_parameters:
                    self.return_params.append((num_rounds, parameters_to_ndarrays(self.parameters)))
        # Bookkeeping
